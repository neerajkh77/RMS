...
class Menu:
  ...
  def edit(self):
    ...
    Entry(self.edit_wind, textvariable=StringVar(self.edit_wind, value=old_name), state='readonly').grid(row=0, column=2, padx=10, pady=5)
    ...
    Entry(self.edit_wind, textvariable=StringVar(self.edit_wind, value=old_price), state='readonly').grid(row=2, column=2, padx=10, pady=5)
    Label(self.edit_wind, text="New Price").grid(row=3, column=1, padx=10, pady=5)
    new_price = Entry(self.edit_wind)
    new_price.grid(row=3, column=2, padx=10, pady=5)

    update_button = ttk.Button(self.edit_wind, text="Save Changes", command=lambda: self.edit_records(new_name.get(), new_price.get(), id))
