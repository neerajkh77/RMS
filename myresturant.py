...
class Menu:
  ...
  def __init__(self, wind):
    self.wind = wind
    self.wind.title("Neeraj Ko Restaurant")
  ...
  def view_orders(self):
    ...
    query = 'SELECT orders.id, orders.quantity, menu.name, menu.price FROM orders INNER JOIN menu ON customer_orders = menu.id WHERE orders.table_number = 1 ORDER BY orders.id DESC'
    db_rows = self.run_query(query)
    for row in db_rows:
  ...
  def view_bills(self):
    query = 'SELECT orders.quantity, menu.price FROM orders INNER JOIN menu ON customer_orders = menu.id WHERE orders.table_number = 1'
    db_rows = self.run_query(query)
    total = 0
    for row in db_rows:
      total += row[0] * row[1]
